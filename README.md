...
> Current version: **0.1.0**. The protocol implementation is covered by local simulated TLS tests, but compatibility should still be validated against each physical device and Android TV version.
...
With MSVC, the static build produces `androidtvremote_static.lib`. The shared
build produces `androidtvremote.dll` and its MSVC import library
`androidtvremote.lib`.
...
GitHub Actions first runs a warning-as-error build and the test suite as a code-quality gate. It then builds and tests native x86_64 and ARM64 variants for Linux, macOS, and Windows. The macOS slices are combined into universal static and shared libraries. Successful builds expose the following native libraries as short-lived workflow artifacts:

- Linux: `libandroidtvremote-linux-x86_64.a`, `libandroidtvremote-linux-arm64.a`, `libandroidtvremote-linux-x86_64.so`, and `libandroidtvremote-linux-arm64.so`;
- macOS universal (x86_64 + ARM64): `libandroidtvremote-macos.a` and `libandroidtvremote-macos.dylib`;
- Windows static libraries: `libandroidtvremote-win-x86_64_static.lib` and `libandroidtvremote-win-arm64_static.lib`;
- Windows DLLs and import libraries: `libandroidtvremote-win-x86_64.dll`, `libandroidtvremote-win-arm64.dll`, `libandroidtvremote-win-x86_64.lib`, `libandroidtvremote-win-arm64.lib`, `libandroidtvremote-win-x86_64.dll.a`, and `libandroidtvremote-win-arm64.dll.a`.

Pushing a tag named `v*` creates or updates a GitHub release containing these files as directly downloadable assets.
