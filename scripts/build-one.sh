...
case "$OS:$KIND" in
    ...
    windows:shared)
        ...
        import_lib="$(find "$BUILD_DIR" -type f \( -name 'libandroidtvremote.dll.a' -o -name 'androidtvremote.lib' \) -print | head -n1)"
        if [ -n "$import_lib" ]; then
            cp -L "$import_lib" "$DIST/libandroidtvremote-${ARCH}.win.dll.a"
            printf '%s\n' "$DIST/libandroidtvremote-${ARCH}.win.dll.a"
        fi
        ;;
    macos:static)
        src="$(find_one 'libandroidtvremote.a')"
