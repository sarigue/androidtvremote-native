...
bool generateCertificateIfMissing(
    ...
    _wfopen_s(&certFile, certificateFile.c_str(), L"wb");
    _wfopen_s(&keyFile, privateKeyFile.c_str(), L"wb");
#else
    FILE* certFile = std::fopen(certificateFile.c_str(), "wb");
    FILE* keyFile = std::fopen(privateKeyFile.c_str(), "wb");
#endif
    if (!certFile || !keyFile) {
        if (certFile) std::fclose(certFile);
        if (keyFile) std::fclose(keyFile);
        throw std::runtime_error("failed to open certificate output files");
    }

    const bool certOk = PEM_write_X509(certFile, certificate.get()) == 1;
    const bool keyOk = PEM_write_PrivateKey(keyFile, key.get(), nullptr, nullptr, 0, nullptr, nullptr) == 1;
    std::fclose(certFile);
    std::fclose(keyFile);
    if (!certOk || !keyOk) {
